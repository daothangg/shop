...
@section('content')
    <div class="page-wrapper">
        <div class="page-content">
            ...
            <hr />
            <div class="col"><button type="button" onclick="saveData('0','','','')"
                    class="btn btn-outline-info px-5 radius-30" data-bs-toggle="modal" data-bs-target="#exampleModal">Add
                    Category</button></div>
            <div class="card">
                <div class="card-body">
                        ...
                        <table id="example2" class="table table-striped table-bordered">
                            ...
                            <tbody>
                                @foreach ($data as $list)
                                    <tr>
                                        <td>{{ $list->id }}</td>
                                        <td>{{ $list['attribute']->name }}({{ $list['attribute']->slug }}) </td>
                                        <td>{{ $list['category']->name }}({{ $list['category']->slug }})</td>

                                        <td><button type="button"
                                                onclick="saveData('{{ $list->id }}','{{ $list->attribute_id }}','{{ $list->category_id }}')"
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-tile" id="exampleModalLabel">Category</h5>
                ...
                <form id="formSubmit" action="{{ url('admin/update_category_attribute') }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="border p-4 rounded">
                            ...
                            <div class="row mb-3">
                                <label for="enter_text" class="col-sm-3 col-form-label">Parent Attribute Id</label>
                                <div class="col-sm-9">
                                    <select class="form-control" name="attribute_id" id="parent_attribute_id">
    ...
    <script>
        ...
        function saveData(id, attribute_id, category_id) {
            if (checkId != 0) {
                $('#parent_category_id option[value="' + checkId + '"]').show();
                $('#parent_attribute_id option[value="' + checkId + '"]').show();
            ...
            $('#parent_attribute_id').val(attribute_id);

            $('#parent_category_id option[value="' + id + '"]').hide();
            $('#parent_attribute_id option[value="' + id + '"]').hide();
